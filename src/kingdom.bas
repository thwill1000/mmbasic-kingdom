...
50:
Mode 7
Cls : On Error Goto 800
procVARIABLE : procTITLEPAGE
procINSTRUCTIONS
200:
F = 5000 + Rnd(2000)
P = 300 + Rnd(100)
...
procMAP : procDBL(S$(S) + " Season             Year " + Str$(Y), 1, 1)
If Rnd(2) = 1 Then Goto 380
procFLOOD : procATTACK : Goto 400
380:
procATTACK : procFLOOD
400:
procCALCULATE
procENDSEASON
...
P = Int(P * 1.045) : Goto 300
500:
VDU 26 : Cls : Print Tab(0, 9)
Print "Press the RETURN key to start again." : ?
Print "Press the ESCAPE key to leave the" : ? "program."
...
Sub procINSTRUCTIONS
  procYELLOW : ? : ? : ? : ?
  Print "The kingdom is three villages. It" : ? "is between the Yellow River and" : ? "the mountains."
  ...
  procSPACE
  Cls : ? : ? : ?
  Print "The year consists of three long " : ? "seasons, Winter, Growing and" : ? "Harvest. Rice is planted every" : ? "Growing Season. You must decide" : ? "how much is planted."
...
Sub procNEWSEASON
  procYELLOW : Print Tab(8, 1); "Census Results" : ? : If J = 1 Then Goto 3050
  Print "At the start of the "; S$(S); " Season"
  Print "of year "; Y; " of your reign this is"
  Print "the situation." : Goto 3100
3050:
...
3100:
  ...
  QU = 14 : A = 0
3210:
  Print Tab(26, QU); : NI = FNNUMINP
  If A + NI > P Then procIMPOS : Goto 3210
  QU = QU + 1 : If QU = 16 Then B = NI : Goto 3260
  A = NI : If A < P Then Goto 3210
  B = 0 : Print Tab(26, QU); B : QU = 16
3260:
  C = P - (A + B) : Print Tab(26, QU); C
  If S <> 2 Then Goto 3390
  ? : ? "How many baskets of rice will be"
  Print "planted in the fields....."
3330:
  Print Tab(26, 19); : G = FNNUMINP
...
Sub procENDSEASON
  ...
  Cls : Print Tab(3, 7); "There was no food left.All of the"
  Print "   people have run off and joined up"
  Print "   with the thieves after "; J; " seasons"
  Print "   of your misrule" : procSPACE : Exit Sub
3600:
  If P > 0 Then Goto 3700
  Cls : Print Tab(2, 8); "There is no-one left! They have all"
  Print "  been killed off by your decisions "
  Print "  after only "; Y; " year"; : If Y <> 1 Then Print "s";
  Print "." : procSPACE : Exit Sub
3700:
  F1 = P / (FD + TD + ST + 1) : F2 = F / (TF + FF + 1) : If F2 < F1 Then F1 = F2
  ...
  procWAIT(2) : Exit Sub
3800:
  procYELLOW : Print Tab(3, 2); "Village Leader's Report"
  ...
  Print "Baskets of rice" : ? "lost during the floods......"; FF
  Print "Baskets of rice" : ? "lost during the attacks....."; TF
  ? : ? "The village census follows."
  procSPACE
End Sub

Sub procADDTHIEVES
  procYELLOW : Print Tab(0, 8)
...
Sub procRITUAL
  ...
  Print "question:" : ? : PROCWAIT(5)
  Print "Are you prepared to accept"
  Print "the burden of decision again?" : ?
  Print "You need only answer Yes or No"
  Print "for the people will understand" : ? "your reasons."
4670:
  Print Tab(0, 21); : procYESORNO : If Y% < 0 Then Goto 4670
End Sub
...
5100:
  V = Rnd(3) : If FL(V) = 1 Then Goto 5100
  X = 32 : WX = VX(V) : WY = VY(V) - 1
  If WY < 17 Then Y = 13 : D = -1 Else Y = 17 : D = 1
  SY = Y
5140:
  Print Tab(X, Y); " " : If Y = WY Then Goto 5160
  Y = Y + D : Print Tab(X, Y); "T" : procWAIT(.05) : Goto 5140
5160:
  X = X - 1 : Print Tab(X - 1, Y); R$; "T" : procWAIT(1 - (X - WX) / 5)
  Print Tab(X, Y); : If X = 29 Then Print "x" Else Print " "
  If X > WX Then Goto 5160
  For I = 1 To 99
    Print Tab(X, Y + 1); Chr$(Rnd(94) + 32) : Next
  procVDRAW(V)
5300:
  X = X + 1 : If X < 27 Then Print Tab(X - 2, Y); " "
  If X = 31 Then Print Tab(29, Y); "x"
  Print Tab(X - 1, Y); R$; "T" : procWAIT(.04)
  If X < 32 Then Goto 5300
5340:
  If Y = SY Then Goto 5400
  Print Tab(X, Y); " " : Y = Y - D : Print Tab(X, Y); "T" : procWAIT(.05) : Goto 5340
5400:
  On S Goto 5410, 5420, 5430
5410:
  I = 200 + Rnd(70) - C : Goto 5440
5420:
  I = 30 + Rnd(200) - C : Goto 5440
5430:
  I = Rnd(400) - C
5440:
  I = Int(I) : If I < 0 Then I = 0
...
Sub procFLOOD
  Local X, Y
  ZF = 0 : On S Goto 5530, 5540, 5550
5530:
  FS = Rnd(330) / (A + 1) : Goto 5560
5540:
  FS = (Rnd(100) + 60) / (A + 1) : Goto 5560
5550:
  Exit Sub
5560:
  If FS < 1 Then Exit Sub
  X = 6 : ZF = 1 : Y = Rnd(8) + 10 : If FS < 2 Then FS = Rnd(2) Else FS = Rnd(4)
...
6050:
  ...
  F = F + Int(G)
6100:
  ST = 0 : P = A + B + C : If P = 0 Then Goto 6299
  T = F / P : If T > 5 Then T = 4 : Goto 6200
  If T < 2 Then P = 0 : Goto 6299
  If T > 4 Then T = 3.5 : Goto 6200
  ST = Int(P * (7 - T) / 7) : T = 3
6200:
  P = P - ST : F = Int(F - P * T - ST * T / 2)
  If F < 0 Then F = 0
6299:
End Sub
...
Sub procVARIABLE
  ...
  S$(1) = "Winter" : S$(2) = "Growing" : S$(3) = "Harvest"
  W$ = Chr$(255) : Y$ = Chr$(&h93) : R$ = Chr$(&h91) : V$ = Chr$(&h92)
  VX(1) = 13 : VY(1) = 8
  VX(2) = 21 : VY(2) = 12
  VX(3) = 22 : VY(3) = 18
End Sub
...
Sub procSPACE
  Print Tab(0, 22); "Press the SPACE BAR to continue"; : procKCL
  REPEAT Until GET$ = " "
End Sub

Sub procTITLEPAGE
  procMAP : procOFF : procWAIT(2) : Print Tab(0, 11); Space$(200)
  procDBL(Y$ + "YELLOW RIVER", 11, 11) : procDBL(Y$ + "KINGDOM", 13, 14)
  I% = INKEY(500)
End Sub
...
Sub procGPI(F2, ML) : ' General Purpose Input Routine
  Local B, B$
  A$ = ""
  Print String$(ML, " "); String$(ML + 1, Chr$(8)); Chr$(&h83); : procON : procKCL
9040:
  B$ = GET$ : B = Asc(B$) : If B = 13 Then Goto 9190
